...
class FileBulkDataElement extends BasicDicomElement {
    ...
    public void writeValueTo(DicomOutputStream dos) throws IOException {
        ...
        try (InputStream in = Files.newInputStream(file)) {
            transfer(in, valuePos, buffer, null);
            if (valueLength != -1) {
                transfer(in, valueLength & 0xffffffffL, buffer, dos);
            } else {
                in.read(buffer, 0, 8);
                while (ByteOrder.LITTLE_ENDIAN.bytesToTag(buffer, 0) == Tag.Item) {
                    dos.write(buffer, 0, 8);
                    transfer(in, ByteOrder.LITTLE_ENDIAN.bytesToInt(buffer, 4) & 0xffffffffL, buffer, dos);
                    in.read(buffer, 0, 8);
                }
            }
        }
    ...
    private void transfer(InputStream in, long length, byte[] buffer, OutputStream out) throws IOException {
        int read;
